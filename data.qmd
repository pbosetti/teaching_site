...
Each data file can be separately loaded from the URLs reported [down below](#list-of-csv-files). Nonetheless, you can also exploit the ability of R `read.*` functions to open a file directly from its URL. To do so, it can be useful to use the `adas.utils::examples_url()` function that builds the URL based on the file name:

```{r}
examples_url("timeseries.csv")
```

Then, just pipe it with the proper file-reading function, e.g.:

```{r eval=FALSE}
examples_url("timeseries.csv") %>% read_csv()
```

or

```{r eval=FALSE}
examples_url("3dprint.dat") %>% read_table(comment="#")
```
...
```{r}
titanic <- examples_url("train.csv") %>% 
