...
format: 
  pdf: 
    ...
    include-in-header:
      text: |
        % CSS-equivalent names used in \color{} inside math (MathJax accepts them, xcolor does not)
...
{{< qrcode https//pbosetti.github.io/teaching_site/slides/ADAS/1-statistica.html width=500 height=500 >}}
...
-   **Densità di probabilità**: per una v.a. continua, la probabilità di riscontrare esattamente un dato valore è nulla, quindi ci si riferisce ad una probabilità di riscontrare un valore minore (o maggiore) di un dato valore. La densità di probabilità è la derivata di tale probabilità
...
Ancora meglio, per un campione di $N=5000$ elementi:
...
for (n in sample_sizes) {
  means <- c(means, mean(sample(pop, n)))
}
tibble(n=sample_sizes, m=means) %>% ggplot(aes(x=n, y=m)) +
  # geom_line(color=gray(0.5)) +
  geom_point(alpha=0.2) +
  geom_hline(yintercept=mean(pop)) +
  labs(x="Dimensione campione", y=TeX("$\\bar{x}$"))
...
-   probabilità di riscontrare difetti su 1 m di filo, quando ci sono mediamente 3 difetti ogni 100 m: $\lambda = 3/100$
-   probabilità di ricevere telefonate nei prossimi 10' in un centralino che ne riceve mediamente 250 al giorno: $\lambda = \frac{250}{24\cdot 60}/10$
-   probabilità di avere dipendenti malati in stagione di influenza, in cui mediamente 5 dipendenti a settimana sono malati: $\lambda = 5$
:::
:::

::: column
```{r}
tibble(q=1:20, p=dpois(q, 5)) %>%
  ggplot(aes(x=q, y=p)) + 
  geom_col(width=0.1) +
  geom_point() +
  labs(x=TeX("x"), y="p(x)") + 
...
:::{.columns}
:::{.column width="50%"}
* I campioni **C1** e [C3]{.bblue} non avranno valori comuni: è molto probabile che vengano da popolazioni differenti
* I campioni **C1** e [C2]{.bgreen} invece sono più difficili da distinguere
...
$$
t_0 = \frac{\bar{y_2} - \bar{y_1}}{S_p\sqrt{\frac{1}{n_1} + \frac{1}{n_2} }}
$$
...
* È evidente che casi come **C1** vs. [C3]{.bblue} nel grafico precedente avranno un valore di $|t_0|$ più alto che casi come **C1** vs. [C2]{.bgreen} (rapporto tra distanza e varianza)
...
H_1 :~& \sigma^2_1 \neq \sigma^2_2
\end{aligned},~~~F_0=\frac{S^2_1}{S^2_2}\sim\mathcal{F}_{n_1-1, n_2-1}
$$
...
#| exercise: "t_test"
n1=10; n2=10
y1=13.59213; y2=10.30381
s1=4.33921; s2=2.24027
...
:::{ .hint exercise="t_test"}
Calcolare prima $F_0$ per un test della varianza e il suo $p$-value, ricordando che il test della varianza è a due lati.

Poi calcolare $t_0$ e il suo $p$-value, tilizzando il test di Student o di Welch a seconda del risultato del test della varianza.
...
```{webr}
#| exercise: "t_test_sol"
# verifico la omoschedasticità:
F0=s1^2/s2^2
2*pf(F0, n1-1, n2-1, lower.tail=F)
# p-value è > 0.05, quindi campioni omoschedastici

Sp = sqrt(((n1-1)*s1^2 + (n2-1)*s2^2)/(n1+n2-2))
T0 = (y1-y2)/(Sp*sqrt(1/n1 + 1/n2))
2*pt(T0, df=n1+n2-2, lower.tail=F)
# quindi µ1 != µ2, con p-value = 0.0473
```
:::
...
::: {.callout-tip title="Demo interattiva"}
Su [https://pbosetti-anova-live.share.connect.posit.cloud](https://pbosetti-anova-live.share.connect.posit.cloud){target="shiny"} è possibile interagire con un'applicazione che consente di simulare esperimenti a un fattore e più livelli, e di calcolare la tabella ANOVA.
:::
...
## Il significato del $p$-value {.smaller}
...
**Nota 2**: Vedi la versione interattiva qui: [https://p4010.shinyapps.io/frequentista/](https://p4010.shinyapps.io/frequentista/){target="_blank"}
:::
