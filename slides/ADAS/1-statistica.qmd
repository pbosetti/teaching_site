...
Ho due campioni di dati, uno con $n_1=`r length(d$c1)`$, $\mu_1 = `r mean(d$c1) |> round(5)`$, $\sigma_1=`r sd(d$c1) |> round(5)`$ e l'altro con $n_2=`r length(d$c2)`$, $\mu_2 = `r mean(d$c2) |> round(5)`$, $\sigma_2=`r sd(d$c2) |> round(5)`$. Voglio confrontare le medie dei due campioni con un test di Student, assumendo $\alpha=0.05$:
...
set.seed(2)
mu0 = 10.3
pop <- rnorm(1000, mean=mu0, sd=2.7)
N <- 1000
alpha <- 0.05
```
...
:::columns
:::column
Il significato del $p$-value è illustrato con un esempio: si consideri un esperimento in cui si effettuano `r N` test di Student, ciascuno con un livello di significatività $\alpha$ del `r alpha*100`%
...
tibble(
  i = 1:N,
  p = map_dbl(1:N, ~t.test(sample(pop, 15), mu=mu0, conf.level=1-alpha)$p.value),
