...
- [Quella della seconda serie è un quarto di quella della prima]{.correct}
...
-   Si dice che $x\sim\mathrm{Binom}(n,p_s)$ o anche $x\sim\mathcal{B}(n,p_s)$ quando la PDF è: 
$$
...
-   Si dice che $x\sim\mathrm{Poisson}(\lambda)$ oppure $x\sim\mathcal{P}(\lambda)$ quando la PDF è: 
$$
...
-   Si dice che $x\sim\mathrm{Geom}(p_s)$ oppure $x\sim\mathcal{G}(p_s)$ quando la PDF è: 
$$
...
ggplot() +
  ...
  labs(x=TeX("x"), y=TeX("CDF(x)"), color="coda:") +
  labs(title="Norm(0,1)")
```
...
Nel gioco del lotto, la probabilità di vincere giocando dempre gli stessi numeri:
...
- Per ogni CDF esistono una o più PDF che la generano
...
-   Si è visto che media e varianza campionaria sono due **stimatori** e rappresentano della **variabili aleatorie**
...
Per i due campioni $y_{1,1}$ e $y_{2,i}$ è possibile definire la variabile:
$$
...
```
:::
Ad esempio, per un campione con 8 g.d.l. e $\alpha=10%$ risulta un valore critico di $t_0=`r round(qt(0.05, 8, lower.tail=F), 3)`$: ogni valore di $t_0$ calcolato superiore a tale valore (in modulo) comporta il rifiuto di $H_0$ con una probabilità d'errore inferiore al 10%
...
Dato parametro ignoto $\vartheta$, vogliamo definire due statistiche $L$ e $U$, tali per cui la probabilità: $P(L\leq\vartheta\leq U)= 1-\alpha$. 
...
Ho due campioni di dati, uno con $n_1=`r length(d$c1)`$, $\mu_1 = `r mean(d$c1) |> round(2)`$, $\sigma_1=`r sd(d$c1) |> round(2)`$ e l'altro con $n_2=`r length(d$c2)`$, $\mu_2 = `r mean(d$c2) |> round(2)`$, $\sigma_2=`r sd(d$c2) |> round(2)`$. Voglio confrontare le medie dei due campioni con un test di Student, assumendo $\alpha=0.05$:

- [$\mu_1=\mu_2$, con un $p$-value pari a `r pe$p.value`]{.correct}
- [$\mu_1\neq \mu_2$, con un $p$-value pari a `r pd$p.value`]{data-explanation="I due campioni hanno la stessa varianza"}
- [$\mu_1\neq \mu_2$, con un $p$-value pari a 0.062316]{data-explanation="Hai sparato... e poi 0.06 > 0.05!"}
- [$\mu_1 = \mu_2$, con un $p$-value pari a 1.386E-4]{data-explanation="Hai sparato, ammettilo!"}
- [$\mu_1\neq \mu_2$, con un $p$-value pari a 0.048]{data-explanation="Non proprio"}

## Calcolatrice

Usare il riquadro sottostante per risolvere l'esercizio precedente:
...
```
Per il numero di canne, o *bin*, si usa la **formula di Sturges**: $K = \lceil\log_2 n \rceil + 1$ o la **formula di Scott**: $K=3.49 s / \sqrt[3]{n}$


## Analisi di normalità: grafico quantile-quantile {.smaller}
...
## Teorema di Cochran
La definizione di $F_0$ presuppone che $MS_{tr}$ e $MS_E$ **siano indipendenti**. Dato che sono il risultato della decomposizione di $$SS_T$$ ciò non è scontato. Vale però il **teorema**:
...
:::columns
:::column
Nell'[esperimento sopra riportato](#esperimento-a-un-fattore-e-piu-livelli), una tabella ANOVA come la seguente è corretta oppure no?
:::
