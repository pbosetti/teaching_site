...
{{< qrcode https//pbosetti.github.io/teaching_site/slides/ADAS/7-serie_temporali.html width=500 height=500 >}}
...
Quindi ad esempio la differenziazione $\nabla^2x_t$ è l'equivalente discreto della derivata seconda $\frac{d^2}{d t^2}x(t)$ per la funzione continua $x(t)$
...
In generale, quando $E(\nabla^d x_t)=0$ il processo $\mathrm{ARIMA}(p,d,q)$ può essere scritto come:
$$
\Phi_p(B)\nabla^d x_t=\Theta_q(B)w_t
$$
Se invece il valore atteso $E(\nabla^d x_t)=\mu$, allora:
$$
\Phi_p(B)\nabla^d x_t=\delta + \Theta_q(B)w_t,~~~\delta=\mu(1-\phi_1-\dots-\phi_p)
$$
