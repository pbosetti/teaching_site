...
The **power** of a test is the error probability in rejecting $H_0$ when it is false, i.e. $1-\beta$. It represents the test **reliability**. 
It depends on the samples size and on the **relative difference** $\delta = (\mu_1 - \mu_2) / \sigma$. It is possible to calculate the **power curves** for different sample sizes and for different values of the **relative difference** $\delta = (\mu_1 - \mu_2) / \sigma$.
...
expand_grid(
  ...
  labs(y="Power", x=TeX("$\\delta=(\\mu_1 - \\mu_2)/\\sigma$")) +
...
df %>% 
  ...
  labs(x="sample size", y="bins count", color="method")
```
...
:::columns
:::column
The meaning of the $p$-value is illustrated with an example: consider an experiment in which `r N` Student tests are performed on as many samples of $n=`r n1`$ elements, each with a significance level of $\alpha = `r alpha*100`$%
...
Then the expected value of the wrong predictions (i.e. those in which $H_1$ holds) is `r alpha*100`%
:::
...
tibble(
  ...
  scale_color_manual(values=c("H0"="darkgreen", "H1"="red"), name="Hypothesis") +
  scale_shape_manual(values=c("H0"=19, "H1"=17), name="Hypothesis") +
  geom_label(x=0, y=alpha, label=glue("{alpha*100}%")) +
  labs(
    title=glue("p-value for {N} T-tests ({length(.$hyp[.$hyp == 'H1'])/N*100}% Type I errors)"), 
    x="# Tests", 
    y="p-value")}
```
...
:::aside
**Note**: The same **frequentist** approach can be applied to any statistical test that produces a $p$-value
:::
...
:::column
If we increase the size of the samples from `r n1` to `r n2`, the probability of a type I error remains the same, under the same conditions
...
:::column
Consider the tests in which the hypotheses ar
...
\end{eqnarray}
$$
i.e. a difference of one $\sigma$ from the expected value of the population
...
tibble(
  ...
  ylim(0,0.5) +
  labs(
    title=glue("p-value for {N} T-tests, sambles with {n3} elements"), 
    x="# Tests", 
    y="p-value")}
```
